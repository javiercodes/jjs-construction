    ...
    >
      <div className="container-custom mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-20">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-1 min-h-[44px] min-w-[44px]">
            <span className="font-heading text-xl md:text-2xl font-bold">
              <span className="text-accent">JJ&apos;s</span>{" "}
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`font-heading text-sm font-semibold tracking-wide uppercase transition-colors duration-200 min-h-[44px] flex items-center ${
                  pathname === link.href
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a
              ...
              className="flex items-center gap-2 text-primary font-semibold hover:text-accent transition-colors min-h-[44px]"
            >
          ...
          <div className="flex lg:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-charcoal min-h-[44px] min-w-[44px] flex items-center justify-center"
        ...
        <nav className="bg-white border-t border-gray-100 px-4 py-4 space-y-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`block py-3 px-4 rounded-lg font-heading font-semibold text-sm uppercase tracking-wide min-h-[44px] transition-colors ${
                pathname === link.href
